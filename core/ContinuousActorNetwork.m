classdef ContinuousActorNetwork < handle
    ...
    methods
        function obj = ContinuousActorNetwork(inputSize, outputSize, layerSizes)
            ...
            % 初始化均值输出层
            obj.learnables.meanw = dlarray(initializeGlorot(outputSize, layerSizes(end)), 'CB');
            obj.learnables.meanb = dlarray(zeros(outputSize, 1), 'CB');
            
            % 初始化方差输出层 (log of std)
            obj.learnables.logstdw = dlarray(initializeGlorot(outputSize, layerSizes(end)), 'CB');
            obj.learnables.logstdb = dlarray(zeros(outputSize, 1), 'CB');
        end
